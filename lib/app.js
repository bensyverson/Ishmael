...
App.prototype.packAndShipFromPath = function(aPath, cb) {
	...
	aVC.view.renderHTML(function(err, html){
		...
		if (err) {
			...
			myHTML = html 
					+ '<script type="text/javascript">var Representable=Representable || null;'
					+ '(function(){function m(){'
					+ 'var p=' + JSON.stringify(self.requirePaths) + ';'
					// + 'window.UserDefaults = null;'
					+ 'if (Representable == null){'
					+ 'for (var i=0;i<p.length;i++){'
					+ ' try{Representable=require(p[i] + "ishmael.js");window.UserDefaults=require(p[i] + "userdefaults.js");}catch(err){}'
