...
/** @const {string} */ var IDControlEventGeneric = 'com.bensyverson.control.genericEvent';
 ...
 * @return
 */
Control.prototype.sendActionsForControlEvents = function(controlEvents, userInfo)
{
	var self = this;
						...
						for (var actionKey in aTarget) {
							if (aTarget.hasOwnProperty(actionKey)){
								...
								if (typeof(targetObject[actionKey]) === typeof(function(){}) ) {
									var anEvent = new IDEvent(IDControlEventGeneric, controlEventMask, userInfo);
...
Control.prototype.didReceiveTouch = function(anEvent) {
	...
	if (self.touchesBeganWithEvent) {
		...
		if (self.touchesMovedInsideWithEvent) {
			/**
			 * Description
			 * @param {} moveEvent
			...
			handleTouchMove = function(moveEvent) {
				return self.touchesMovedInsideWithEvent(moveEvent);
			};
			element.addEventListener('touchmove', handleTouchMove, false);
			element.addEventListener('mousemove', handleTouchMove, false);
		} else if (self.touchesMovedWithEvent) {
			/**
			 * Description
			 * @param {} moveEvent
			 * @return CallExpression
		...
		if (self.touchesEndedWithEvent) {
			...
			handleTouchEnd = function(endEvent) {
				...
				element.removeEventListener('mousemove', handleTouchMove, false);
				...
				window.removeEventListener('mousemove', handleTouchMove, false);
