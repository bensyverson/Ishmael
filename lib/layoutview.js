...
var NativeDOMUtils = NativeDOMUtils || require('./domutils.js');
var LayoutCache = LayoutCache || require('./layoutcache.js');
...
var AutoLayout = function(options) {
	...
	if (options) {
		...
		this.setControlActions = options.setControlActions || this.setControlActions;
	}
...
AutoLayout.prototype.printSubviews = function(v) {
	...
	var innerPrintSubviews = function(aView, anIndent) {
		...
		console.log(anIndent + aView.templateConst);
...
AutoLayout.prototype.attribsFromView = function(node){
	...
	var attribs = self.domUtils.getAttribs(node);
	var newAttribs = {
		visibility: self.visibilityOfNode(node, attribs),
		className: null,
		name: null,
		target: null,
		action: null,
	...
	if (attribs) {
		newAttribs.className 	= (attribs['data-ish-class']) ? attribs['data-ish-class'] : null;
		newAttribs.name 		= (attribs['data-ish-name']) ? attribs['data-ish-name'] : null;
		newAttribs.target 		= (attribs['data-ish-target']) ? attribs['data-ish-target'] : null;
		newAttribs.action 		= (attribs['data-ish-action']) ? attribs['data-ish-action'] : null;
		newAttribs.events 		= (attribs['data-ish-event']) ? attribs['data-ish-event'] : null;
	}
	return newAttribs;
}
...
AutoLayout.prototype.viewFromNode = function(node, parentView) {
		...
		if (className !== 'View') {
			...
			if (aView === null) {
				for (var j = 0; j < self.requirePaths.length; j++) {
					if (aView !== null) break;
					try {
						var requiredClass = require(self.requirePaths[j] + className + '.js') || null;

						if (requiredClass !== null) {
							aView = new requiredClass(null, attribs.name);
						}
					} catch(e) {
						// console.log("Tried to instantiate custom class: " + e);
					}
				}
			}
			if (aView === null) {
				// console.log("Couldn't instantiate custom class: " + className);
			}
		}
	}

	if (aView === null) {
		var _View = View || ctx['View'] || null;
		if (!_View) {
			for (var j = 0; j < self.requirePaths.length; j++) {
				try {
		...
		if (_View) {
			aView = new _View(null, attribs.name);
		} else {
			console.log("Couldn't instantiate basic View!");
		}
	}
	...
	// If we can, let's autowire any instance variables with our name, so that a superview with a variable `self.buyButton` is wired up to a subview with the name `buyButton` automatically. 
	if (self.setInstanceVariables && instanceName) {
		var aParent = parentView;
		while (aParent) {
			if (!(aParent[instanceName] === undefined) &&
				!(aParent[instanceName] instanceof Function)) {
	...
	// If we can, let's autowire any events this view is emitting, so its superviews can be notified.
	if (self.setControlActions && attribs['events'] && (Control[attribs['events']] !== typeof(undefined))) {
		var events = Control[attribs['events']];

		// We can either attach to a known target, or we'll just send an event to the superview.
		var knownTarget = (attribs['target'] != null);
		// We can specify a method using `data-ish-action`, but otherwise we'll use View's `didReceiveEvent`
		var knownAction = attribs['action'] || 'didReceiveEvent';

		// First, set the target to our parent view (the default)
		var aParent = parentView;
		// If we have a known target, go looking for it.
		if (knownTarget) {
			while (aParent) {
				// If this parent view has a method in its prototype equal to `knownAction`, we're in business.
				if (aParent && (typeof(aParent[knownAction]) === typeof(function(){}))) {
					// Found it!
					break;
				}
				aParent = aParent.superview;
		...
		if (aParent == null) aParent = parentView;

		// Wire it up.
		aView.addTargetActionForControlEvents(aParent, knownAction, events);
	}
		...
		for (var i = 0 ; i < nodeChildren.length; i++) {
			...
			if ((i < first) || (i > last)) {
				...
				if (subChildCount > 0) {
					...
					// console.log(self.domUtils.getOuterHTML(child));
...
AutoLayout.prototype.autoLayoutViewWithHTML = function(aView, someHtml, selector, normalizeWhitespace) {
	...
	var html = someHtml.trim();

	var dom =  self.domUtils.parseDOM(html, {normalizeWhitespace: normalizeWhitespace ? true : false});
