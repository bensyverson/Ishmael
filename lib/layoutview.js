 ...
 *
 * al.attribsFromView(nodeWithView).className // => "TestView"
 * al.attribsFromView(nodeWithView).name // => "Example"
 * al.attribsFromView(nodeWithView).visibility // => "visible"
 * al.attribsFromView(hiddenNode).className // => "EdView"
 * al.attribsFromView(hiddenNode).name // => "Ed"
 * al.attribsFromView(hiddenNode).visibility // => "hidden"
 * al.attribsFromView(al.emptyDiv).className // => null
 * al.attribsFromView(al.emptyDiv).name // => null
 * al.attribsFromView(al.emptyDiv).visibility // => "visible"
 */
			...
			if (aView === null) {
				for (var j = 0; j < self.requirePaths.length; j++) {
					...
					try {
						...
						//print("Tried to instantiate custom class: " + e);
					}
				}
			}
			if (aView === null) {
