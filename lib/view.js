...
var OrgStuffHereQueue = OrgStuffHereQueue || PutStuffHere.queue;
var Control = Control || require('./control.js');
var findPos = findPos || require('./common.js').findPos;
var CGRect = CGRect || require('./common.js').CGRect;
...
View.prototype.elementRect = function(){
	var self = this;
	var el = self.element();
	var point = findPos(el);
	return new CGRect(point.x, point.y, el.clientWidth, el.clientHeight);
};
...
View.prototype.init = function(cb) {
	...
	if (fullyInitialized == true) {
		// self.printSubviews('');
		if (cb instanceof Function) cb(null, self.uniqueId());
		return;
	}
...
View.prototype.autoLayout = function(html, selector) {
	...
	if (self.template === null) {
		var AutoLayout = global.AutoLayout || require('./layoutview.js');
		...
		if (self.templateName) {
			self.app.templateCache[self.templateName] = self.templateConst;
			self.templateConst = null;
		}
	}
};
...
View.prototype.initializeSubviews = function(cb){
	...
	if (self.identity == 'DetailListView') console.log("Initializing subviews of " + self.identity() + " : " +  self.uniqueId());
...
View.prototype.bindToAppElement = function(anApp, anElement, cb) {
	...
	console.log("BINDING root view (" + self.uniqueId() + ")");
	self.renderHTML(function(err, html){
		console.log("************** BIND IS FINALLY DONE.");
...
View.prototype.layoutSubviews = function() {
	...
	// console.log("Laying out subviews for " + self.identity() + " : " + self.uniqueId());
...
View.prototype.update = function(cb) {
	...
	if ((!self.initialized) && (self.initStarted)) {
		//if (cb instanceof Function) self.enqueue(cb);
		if (cb instanceof Function) cb();
		return;
	}
	...
	return self.createInitLayoutSubviews(function(err, anId){
		...
		if (cb instanceof Function) cb(err, self.uniqueId());
	});
};
...
View.prototype.renderHTML = function(cb) {
	...
	return self.createInitLayoutSubviews(function(err, anId){
		self.printSubviews('');
		// console.log("============== RENDER TOTALLY DONE FOR " + self.uniqueId());
		if (cb instanceof Function) cb(null, self._render());	
	});
};
